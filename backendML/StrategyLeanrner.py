...
class StrategyLearner(object):
    ...
    '''
        train the model
        regard daily return of portval as reward
        modify q value according to action state pair and reward
    ''' 
    ...
    '''
        compute discreate state value according to indicators value using threshold map
        call the function when add evidence and testing
        bbv, k, vol => 0~999
    '''
    ...
    then investing in 500 shares of the relevant symbol and holding 
    that position
    '''
    def addBench(self, df, symb, sv):
        df['bench'] = sv-df.loc[df.index[0],symb[0]]*500
        df['bench'] += 500*df[symb[0]]
        return df['bench']



    '''
    print out rubric value for portfolio and benchmark
    '''
    def rubric(df, col):
        port_val = df[col]
